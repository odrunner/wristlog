...
function renderModelPage(el, ctx, h) {
  ...
  if (tab === 'data') {
    ...
    if (mv.length) parts.push(sect(`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;"><span class="mp-h" style="color:var(--gold);">Member-verified${st.specs_gen ? ` · ${escHtml(st.specs_gen)}` : ''}</span><span class="mp-badge">Exclusive</span></div>
