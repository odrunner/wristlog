...
function renderModelPage(el, ctx, h) {
  ...
  if (tab === 'data') {
    ...
    const parts = [];
    if (refRows.length) parts.push(`<div><div class="mp-h" style="margin-bottom:10px;">Reference spec</div>
        <div style="display:grid;grid-template-columns:auto 1fr;gap:0 12px;font-size:12px;">${refRows.map(([l, v], i) => { const bt = i ? 'border-top:1px solid var(--border);' : ''; return `<span style="color:var(--muted);padding:7px 0;${bt}">${l}</span><span style="color:var(--text);padding:7px 0;text-align:right;${bt}">${escHtml(v)}</span>`; }).join('')}</div></div>`);
    if (refs.length) parts.push(sect(`<div class="mp-h" style="margin-bottom:6px;">References by era</div>${refs.map(r => `<div style="display:flex;gap:10px;font-size:12px;padding:6px 0;border-top:1px solid var(--border);"><span style="min-width:70px;color:var(--text);">${escHtml(r.reference || '')}</span><span style="min-width:84px;color:var(--muted);">${escHtml(r.years || '')}</span><span style="color:var(--muted);">${escHtml(r.note || '')}</span></div>`).join('')}`, !parts.length));
    if (cals.length) parts.push(sect(`<div class="mp-h" style="margin-bottom:6px;">Calibers by era</div><div style="display:flex;flex-wrap:wrap;gap:6px;">${cals.map(c => `<span style="font-size:11px;border:1px solid var(--border);border-radius:var(--radius-pill);padding:4px 9px;">${escHtml(c.caliber || '')}${c.years ? ` <span style="color:var(--muted);">${escHtml(c.years)}</span>` : ''}</span>`).join('')}</div>`, !parts.length));
    if (mv.length) parts.push(sect(`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;"><span class="mp-h" style="color:var(--gold);">From members' watches${st.specs_gen ? ` · ${escHtml(st.specs_gen)}` : ''}</span><span class="mp-badge">Exclusive</span></div>
        <div style="display:grid;grid-template-columns:auto 1fr auto;gap:0 12px;font-size:12px;align-items:center;">${mv.map(([k, label], i) => { const bt = i ? 'border-top:1px solid var(--border);' : ''; return `<span style="color:var(--muted);padding:7px 0;${bt}">${label}</span><span style="color:var(--text);padding:7px 0;${bt}">${escHtml(agg[k].v)}</span><span style="color:var(--muted);font-size:9.5px;padding:7px 0;${bt}">${agg[k].n} watch${agg[k].n === 1 ? '' : 'es'}</span>`; }).join('')}</div>`, !parts.length));
    panel = parts.length ? `<div style="display:flex;flex-direction:column;gap:16px;">${parts.join('')}</div>` : `<div style="font-size:12px;color:var(--muted);">No specs on file yet.</div>`;
  } else {
