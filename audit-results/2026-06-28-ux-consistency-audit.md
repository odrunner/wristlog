...
- ✅ **Avatar initials — luminance-based text color (v841).** Added `initialsTextColor(bg)` (YIQ brightness threshold → `#000`/`#fff`), mirrored into `wrotate_test.js` with 5 unit tests, registered in mirror-drift. Routed all ~20 watch-color avatar renders through it; removed leftover dead `#fff`/`#000`. Initials now contrast-correct on any dial color (dark dial → white, light dial → black).
- ✅ **H6 (eyebrow class, partial — v842).** Added `.eyebrow`/`.eyebrow-muted` (.72rem/700/.08em/uppercase). Migrated the cleanest 17 inline section labels (the .72rem/700/.06–.07em gold+muted family) to it, preserving margins (sub-perceptible change). **Remaining ~49 inline labels deferred** — size-divergent (.78/.68/.65rem) and `badge-deep`-colored ones change appearance more, so they want per-section visual review.

- ✅ **M6 (v845)** — feed action-bar icons unified to 15px (comment/share/add-photo were 13px); added 40px min touch target to `.feed-action-btn`/`.feed-edit-btn`/`.btn-icon` in the mobile media query.
- ✅ **L cleanups (v845)** — added `.admin-card` and replaced 13 identical inline admin/info-card blocks; removed the overridden duplicate `.toggle-slider` rule set and the unused `.wpm-hero-avatar` rule.
- ✅ **`.pill` primitive + radii (v845)** — added `.pill`/`.pill-muted`; collapsed the radius drift into two token families: round pills (`.fb-type-badge`/`.feed-badge-ribbon`/`.vis-badge` → `--radius-pill`) and rect tags (`.rec-tag`/`.pi-card-conf`/`.badge-cat-chip` → `--radius-sm`). Kept the two intentional shape languages (round vs tag); did not force all 17 variants into one shape (that's a design call). Inline one-off pills (June-2026 chip, msr-mode-chip) not yet migrated to `.pill` — left for incremental use.

- ✅ **H6 remainder (v846)** — migrated 27 more gold/muted inline section labels to `.eyebrow`/`.eyebrow-muted` (44 total now). Left the 10px micro field-labels, `badge-deep` warm-card labels, the success "June" chip, and dynamic-color labels as intentionally distinct. Added a reusable screenshot harness (`e2e/_screenshots.mjs`) + before/after captures under `ux-screenshots/`.
...
- ✅ **Badge glyphs cream-on-cream (v844):** the medallion disc is fixed cream (`#FEFCF6`) in *both* themes, but the `#3D2A14→var(--badge-text)` tokenization also caught the **glyph strokes/fills** — so in dark mode the glyph went cream-on-cream (invisible). Added `--badge-ink` (`#3D2A14`, **no dark override** since the disc never darkens) and pointed all 33 glyph stroke/fill at it. **Lesson:** a theme-adapting token applied to content on a *fixed-color* surface inverts wrongly — fixed surfaces need fixed-ink tokens.
