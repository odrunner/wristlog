...
- H7 remainder (`.pill` primitive — 17 variants), H5 (spacing-scale migration — shifts pixels), H6 (type scale + `.eyebrow` class, ~100 labels), M2/M3 (one destructive verb + one confirm pattern via `showConfirm()`), M6 (icon-size/hit-area), L1–L8. None are blocking; all are appearance/structure changes that benefit from an eyeball per step.
