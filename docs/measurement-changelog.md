...
| 2026-08-30 | **Self-healing loop shipped** — the Sunday job now judges the running knob trial (50/50 A/B, arm from each session's `[TGTUNE]` echo, all sessions since start) and promotes/reverts/extends it, then starts the best untried whole-era gate candidate; email reports what it did | process | Before/after reads can't separate a bad change from a noisy week (guardmode=1: 34% vs 25% wrong-of-converged the week it went live, with a different user mix) — same-week A/B can | wrong-of-converged (primary); converged% + no-reading% guardrails | trial #1 = `tgknob_guardmode_0` (tests the 08-23 promotion) — judged 2026-09-06 |
