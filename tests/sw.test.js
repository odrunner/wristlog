...
// Read current cache version from sw.js so tests stay in sync after bumps
...
describe('sw.js install', () => {
  it('precaches the expected URLs on install', async () => {
    ...
    expect(mockCache.addAll).toHaveBeenCalledWith(['/', '/index.html', '/manifest.json', '/icon.svg', '/profile/']);
  });
...
describe('sw.js activate', () => {
  it('deletes old caches and claims clients', async () => {
    globalThis.caches.keys.mockResolvedValue([PREV_CACHE, CURRENT_CACHE, 'other-cache']);
    await loadSW();
    await fireEvent('activate');

    expect(globalThis.caches.delete).toHaveBeenCalledWith(PREV_CACHE);
    expect(globalThis.caches.delete).toHaveBeenCalledWith('other-cache');
    expect(globalThis.caches.delete).not.toHaveBeenCalledWith(CURRENT_CACHE);
    expect(globalThis.self.clients.claim).toHaveBeenCalled();
  });

  it('does not delete any caches when only current cache exists', async () => {
    globalThis.caches.keys.mockResolvedValue([CURRENT_CACHE]);
    await loadSW();
    await fireEvent('activate');
