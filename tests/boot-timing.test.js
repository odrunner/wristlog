...
describe('bootTimingPayload', () => {
  ...
  it('rounds every mark to whole milliseconds', () => {
    ...
    expect(p).toMatchObject({
      ...
      sw_controlled: true, cached_feed: true, feed_error: false, optimistic_boot: false, early_feed: false, social_cache: false, feed_rpc: false,
    });
  });
  ...
  it('survives a missing navigation entry and missing marks', () => {
    ...
    expect(p).toEqual({
      ...
      sw_controlled: false, cached_feed: false, feed_error: false, optimistic_boot: false, early_feed: false, social_cache: false, feed_rpc: false,
    });
  });
  ...
  it('records that the feed came from feed_page()', () => {
    expect(bootTimingPayload({ marks, nav, feedRpc: true }).feed_rpc).toBe(true);
  });

  it('records that the feed started from the remembered follows', () => {
    expect(bootTimingPayload({ marks, nav, socialCache: true }).social_cache).toBe(true);
  });
...
describe('firstLoadCardHtml', () => {
  ...
  it('shows one row per day with median / p90 in seconds and shares in percent', () => {
    ...
    expect(html).toContain('0.8s <span class="text-muted">/ 2.4s</span>');   // page ready
    expect(html).toContain('1.6s <span class="text-muted">/ 4.0s</span>');   // first posts
    expect(html).toContain('2.1s <span class="text-muted">/ 5.2s</span>');   // feed complete
  ...
  it('accepts numeric strings (PostgREST returns numerics as strings) and escapes the day', () => {
    ...
    expect(html).toContain('1.5s <span class="text-muted">/ 3.0s</span>');
