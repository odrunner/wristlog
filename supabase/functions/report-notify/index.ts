...
serve(async (req) => {
  try {
    ...
    const reporterName = reporter?.display_name || reporter?.username || "Unknown";
    const reportedName = reported?.display_name || reported?.username || "Unknown";

    const subject = `[WRotate Report] ${record.reason} — ${record.content_type} by ${reportedName}`;
    const htmlBody = `
      ...
      <table style="border-collapse:collapse;">
        ...
        <tr><td style="padding:4px 12px 4px 0;font-weight:bold;">Content type</td><td>${record.content_type}</td></tr>
        <tr><td style="padding:4px 12px 4px 0;font-weight:bold;">Reason</td><td>${record.reason}</td></tr>
        <tr><td style="padding:4px 12px 4px 0;font-weight:bold;">Details</td><td>${record.details || "None"}</td></tr>
        <tr><td style="padding:4px 12px 4px 0;font-weight:bold;">Time</td><td>${record.created_at}</td></tr>
      </table>
