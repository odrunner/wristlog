...
(async () => {
  ...
  const seen = [];
  p.on('response', r => { const ct = r.headers()['content-type'] || ''; if (ct.startsWith('image/') && !/svg|icon|logo|sprite/i.test(r.url())) seen.push({ u: r.url(), ct, len: +(r.headers()['content-length'] || 0), r }); });
  try {
    await p.goto(page_url, { waitUntil: 'domcontentloaded', timeout: 60000 });
    await p.waitForTimeout(7000);
    let pick = re ? seen.filter(s => re.test(s.u)).sort((a, b2) => b2.len - a.len)[0] : null;   // largest matching variant
    ...
    let buf = null, ctype = pick.ct;
    if (url === pick.u) { try { buf = await pick.r.body(); } catch (e) { buf = null; } }   // bytes already in the browser
    if (!buf) {
      const resp = await ctx.request.get(url, { headers: { Referer: page_url }, timeout: 60000 });
      if (!resp.ok()) { console.log(JSON.stringify({ error: `download ${resp.status()}`, url })); await b.close(); return; }
      buf = await resp.body(); ctype = resp.headers()['content-type'];
    }
    fs.writeFileSync(out, buf);
    console.log(JSON.stringify({ url, bytes: buf.length, contentType: ctype }));
