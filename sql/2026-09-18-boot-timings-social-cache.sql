-- boot_timings.social_cache: true when the feed's first stage started from the
-- follows / blocks / close friends remembered on the device instead of waiting
-- for those lookups (client: restoreSocialCache). Additive; older clients that
-- don't send the field store false.
ALTER TABLE public.boot_timings ADD COLUMN IF NOT EXISTS social_cache boolean;
...
BEGIN
  ...
  INSERT INTO boot_timings (
    ...
    sw_controlled, cached_feed, feed_error, optimistic_boot, early_feed, social_cache)
  SELECT
    v_uid,
