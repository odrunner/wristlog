-- Audit P2: evaluate auth.uid() once per query instead of once per row.
--
-- A bare auth.uid() in a policy is a volatile call Postgres re-runs for EVERY row
-- examined. Wrapped as ( SELECT auth.uid() ) it becomes an InitPlan, evaluated once.
-- Semantically identical - no permission changes at all.
--
-- One transaction PER TABLE: CREATE POLICY takes an exclusive lock, and holding 41
-- of them in a single transaction would stall reads on a 406 MB instance.
--
-- Rollback: sql/2026-08-13-p2-rollback.sql


-- app_feedback (2 policies)
BEGIN;
...
CREATE POLICY "Relevant users can read invites" ON public.club_invites AS PERMISSIVE FOR SELECT TO public
  USING (((( SELECT auth.uid() ) = invitee_id) OR (( SELECT auth.uid() ) = invited_by)));
...
CREATE POLICY "ci_delete" ON public.club_invites AS PERMISSIVE FOR DELETE TO public
  USING (((( SELECT auth.uid() ) = invitee_id) OR is_club_member(club_id)));
...
CREATE POLICY "Relevant users can read join requests" ON public.club_join_requests AS PERMISSIVE FOR SELECT TO public
  USING (((( SELECT auth.uid() ) = user_id) OR (( SELECT auth.uid() ) IN ( SELECT cm.user_id
...
CREATE POLICY "cjr_delete" ON public.club_join_requests AS PERMISSIVE FOR DELETE TO public
  USING (((( SELECT auth.uid() ) = user_id) OR is_club_member(club_id)));
...
CREATE POLICY "Users can join clubs" ON public.club_members AS PERMISSIVE FOR INSERT TO public
  WITH CHECK (((( SELECT auth.uid() ) = user_id) OR (( SELECT auth.uid() ) IN ( SELECT c.created_by
   FROM clubs c
...
CREATE POLICY "Users can leave clubs or owners can remove" ON public.club_members AS PERMISSIVE FOR DELETE TO public
  USING (((( SELECT auth.uid() ) = user_id) OR (( SELECT auth.uid() ) IN ( SELECT c.created_by
   FROM clubs c
  WHERE (c.id = club_members.club_id)))));
...
CREATE POLICY "cm_insert" ON public.club_members AS PERMISSIVE FOR INSERT TO public
  WITH CHECK (((( SELECT auth.uid() ) = user_id) OR is_club_member(club_id)));
...
CREATE POLICY "cm_update" ON public.club_members AS PERMISSIVE FOR UPDATE TO public
  ...
  WHERE ((club_members_1.club_id = club_members_1.club_id) AND (club_members_1.user_id = ( SELECT auth.uid() )) AND (club_members_1.role = 'owner'::text)))))
...
CREATE POLICY "Suspended users cannot create clubs" ON public.clubs AS PERMISSIVE FOR INSERT TO public
  WITH CHECK (((( SELECT auth.uid() ) = created_by) AND (NOT (EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.id = ( SELECT auth.uid() )) AND (profiles.is_suspended = true)))))));
...
CREATE POLICY "Anyone can read comments" ON public.comments AS PERMISSIVE FOR SELECT TO public
  USING (((( SELECT auth.uid() ) = user_id) OR (moderation_status IS NULL)));
...
CREATE POLICY "Owner or post author can delete comments" ON public.comments AS PERMISSIVE FOR DELETE TO public
  USING (((( SELECT auth.uid() ) = user_id) OR (( SELECT auth.uid() ) = ( SELECT logs.user_id
   FROM logs
...
-- email_campaign_sends (1 policies)
BEGIN;
...
CREATE POLICY "Admin full access" ON public.email_campaign_sends AS PERMISSIVE FOR ALL TO public
  USING ((( SELECT auth.uid() ) = 'd70b1a85-4f31-4431-b3b7-db76543daaf5'::uuid));
COMMIT;

-- email_campaigns (1 policies)
BEGIN;
...
CREATE POLICY "Suspended users cannot insert follow_requests" ON public.follow_requests AS PERMISSIVE FOR INSERT TO public
  WITH CHECK (((( SELECT auth.uid() ) = requester_id) AND (NOT (EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.id = ( SELECT auth.uid() )) AND (profiles.is_suspended = true)))))));
...
CREATE POLICY "Suspended users cannot insert follows" ON public.follows AS PERMISSIVE FOR INSERT TO public
  WITH CHECK (((( SELECT auth.uid() ) = follower_id) AND (NOT (EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.id = ( SELECT auth.uid() )) AND (profiles.is_suspended = true)))))));

DROP POLICY IF EXISTS "Users can follow others" ON public.follows;
CREATE POLICY "Users can follow others" ON public.follows AS PERMISSIVE FOR INSERT TO public
  WITH CHECK ((((( SELECT auth.uid() ) = follower_id) OR (( SELECT auth.uid() ) = following_id)) AND (NOT (EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.id = ( SELECT auth.uid() )) AND (profiles.is_suspended = true)))))));
...
CREATE POLICY "users_can_view_own_followers" ON public.follows AS PERMISSIVE FOR SELECT TO public
  USING (((following_id = ( SELECT auth.uid() )) OR (follower_id = ( SELECT auth.uid() ))));
COMMIT;

-- friend_requests (12 policies)
BEGIN;
...
CREATE POLICY "Suspended users cannot insert friend_requests" ON public.friend_requests AS PERMISSIVE FOR INSERT TO public
  WITH CHECK (((( SELECT auth.uid() ) = initiator_id) AND (NOT (EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.id = ( SELECT auth.uid() )) AND (profiles.is_suspended = true)))))));
...
CREATE POLICY "Users can delete own friend requests" ON public.friend_requests AS PERMISSIVE FOR DELETE TO public
  USING (((( SELECT auth.uid() ) = initiator_id) OR (( SELECT auth.uid() ) = target_id)));
...
CREATE POLICY "Others can read shared logs" ON public.logs AS PERMISSIVE FOR SELECT TO public
  USING (((moderation_status IS NULL) AND ((visibility = 'public'::text) OR ((visibility = 'followers'::text) AND (EXISTS ( SELECT 1
   FROM follows
  WHERE ((follows.follower_id = ( SELECT auth.uid() )) AND (follows.following_id = logs.user_id))))) OR ((visibility = 'friends'::text) AND (EXISTS ( SELECT 1
   FROM friend_requests
  WHERE ((friend_requests.status = 'accepted'::text) AND (((friend_requests.initiator_id = ( SELECT auth.uid() )) AND (friend_requests.target_id = logs.user_id)) OR ((friend_requests.target_id = ( SELECT auth.uid() )) AND (friend_requests.initiator_id = logs.user_id))))))) OR ((visibility IS NULL) AND (EXISTS ( SELECT 1
   FROM follows
  WHERE ((follows.follower_id = ( SELECT auth.uid() )) AND (follows.following_id = logs.user_id))))) OR ((club_id IS NOT NULL) AND (visibility IS DISTINCT FROM 'private'::text) AND (EXISTS ( SELECT 1
   FROM club_members
  WHERE ((club_members.club_id = logs.club_id) AND (club_members.user_id = ( SELECT auth.uid() )))))))));
...
-- measurement_batch_runs (1 policies)
BEGIN;
...
CREATE POLICY "mbr_admin_all" ON public.measurement_batch_runs AS PERMISSIVE FOR ALL TO authenticated
  USING ((( SELECT auth.uid() ) = 'd70b1a85-4f31-4431-b3b7-db76543daaf5'::uuid))
  WITH CHECK ((( SELECT auth.uid() ) = 'd70b1a85-4f31-4431-b3b7-db76543daaf5'::uuid));
COMMIT;

-- notifications (12 policies)
BEGIN;
...
CREATE POLICY "Suspended users cannot insert notifications" ON public.notifications AS PERMISSIVE FOR INSERT TO public
  WITH CHECK (((( SELECT auth.uid() ) = actor_id) AND (NOT (EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.id = ( SELECT auth.uid() )) AND (profiles.is_suspended = true)))))));
...
-- review_prompt_events (2 policies)
BEGIN;
...
-- timegrapher_debug_logs (2 policies)
BEGIN;
...
-- timegrapher_results (7 policies)
BEGIN;
...
-- watch_fact_progress (1 policies)
BEGIN;
...
CREATE POLICY "Others can read shared watches" ON public.watches AS PERMISSIVE FOR SELECT TO public
  USING (((watch_privacy = 'public'::text) OR (watch_privacy IS NULL) OR ((watch_privacy = 'followers'::text) AND (EXISTS ( SELECT 1
   FROM follows
  WHERE ((follows.follower_id = ( SELECT auth.uid() )) AND (follows.following_id = watches.user_id))))) OR ((watch_privacy = 'friends'::text) AND (EXISTS ( SELECT 1
   FROM friend_requests
  WHERE ((friend_requests.status = 'accepted'::text) AND (((friend_requests.initiator_id = ( SELECT auth.uid() )) AND (friend_requests.target_id = watches.user_id)) OR ((friend_requests.target_id = ( SELECT auth.uid() )) AND (friend_requests.initiator_id = watches.user_id)))))))));
...
CREATE POLICY "Others can read shared wishlist" ON public.wishlist AS PERMISSIVE FOR SELECT TO public
  USING (((wish_privacy = 'public'::text) OR (wish_privacy IS NULL) OR ((wish_privacy = 'followers'::text) AND (EXISTS ( SELECT 1
   FROM follows
  WHERE ((follows.follower_id = ( SELECT auth.uid() )) AND (follows.following_id = wishlist.user_id))))) OR ((wish_privacy = 'friends'::text) AND (EXISTS ( SELECT 1
   FROM friend_requests
  WHERE ((friend_requests.status = 'accepted'::text) AND (((friend_requests.initiator_id = ( SELECT auth.uid() )) AND (friend_requests.target_id = wishlist.user_id)) OR ((friend_requests.target_id = ( SELECT auth.uid() )) AND (friend_requests.initiator_id = wishlist.user_id)))))))));
