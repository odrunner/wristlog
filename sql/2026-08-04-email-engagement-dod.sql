-- Day-over-day for the admin traffic tab's email metrics.
--
-- Every figure admin_email_engagement returned was all-time (90-day window), so
-- the Email Engagement card and the By Campaign list could not answer "what
-- moved today?" — the one question the rest of that tab already answers via
-- admin_dod_counts. This adds a last-24h counterpart to each engagement figure,
-- per subject and per broadcast label; the client sums them for the headline
-- card exactly as it already sums the all-time ones, so no separate top-level
-- key is needed and the two can never disagree.
--
-- Window is `now() - interval '24 hours'`, identical to admin_dod_counts, so a
-- "+N" reads the same everywhere on the tab.
--
-- opened/clicked stay count(DISTINCT email_id): opens fire per open (prefetch,
-- re-opens), so a raw count inflates the delta the same way it once inflated the
-- rate. Everything else is unchanged from 2026-07-30-email-health.sql.
