-- Audit 2026-09-23 SEC-23-21 — model_stats() is public (model pages, logged-out
-- too) and its value / cost-per-wear medians used every owner's watches,
-- hidden collections included. With one contributor the "median" WAS that
-- owner's figure: purchase price = cost-per-wear median x wears (17 models),
-- market value from a single hidden owner (270 models). Now both need >= 3
-- distinct owners, and a value-history month needs >= 3 owners, not 3 points.
-- Everything else in the function is unchanged (captured from the live DB).
...
begin
  ...
  val_months as (   -- a month is shown only with >=3 distinct owners (SEC-23-21)
    select ym, percentile_cont(0.5) within group (order by price) med, count(*) n
    from pts group by ym having count(distinct user_id) >= 3 order by ym
  ),
  val_now as (
  ...
  select json_build_object(
    ...
    'cost_per_wear', (select case when n >= 3 then json_build_object('median', round(med::numeric, 2), 'n_owners', n, 'wears', wears) end from cpw_agg),
